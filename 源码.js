  ...
  try {
    if (recordType === "all") {
      ...
      return jsonResponse({
        ...
        type: "all",
        status: "success",
        a_records: aResult || { status: "failed" },
        aaaa_records: aaaaResult || { status: "failed" },
      });
    }
...
function buildDnsQuery(domain, recordType) {
  const typeCodes = { A: 1, AAAA: 28, CNAME: 5, NS: 2 };
  let typeCode;
  if (!isNaN(recordType)) {
...
function parseDnsResponse(response, domain, recordType) {
  ...
  if (ancount === 0) {
    return { domain, type: recordType, status: "no_records", answers: [] };
  }
...
function compressIPv6(ip) {
  let p = ip.split(":").map((x) => x.replace(/^0+/, "") || "0"),
    best = [0, 0],
    cur = [0, 0];
  for (let i = 0; i < 8; i++) p[i] === "0" ? cur[1]++ : ((cur = [[i, 0]]), 0);
  for (let i = 0; i < 8; i++) {
    let s = i,
      len = 0;
    while (p[i] === "0") (i++, len++);
...
function parseDnsAnswer(response, offset) {
  ...
  const typeNames = { 1: "A", 28: "AAAA", 5: "CNAME", 2: "NS" };
